        ...
        for file in files:
            if file.endswith('.log'):
                ...
                if not split_test_train:
                    ...
                    csvfile_with_ts = OUTPUT_DIR+app_sys_name+'.csv'
...
def get_clusters(csv_log_file, split_test_train=False):
    ...
    if split_test_train:
        ...
        #y_df = csv_log_df[["label"]]
        #train_df, test_df, y_train, y_test = train_test_split(csv_log_df, y_df, stratify=y_df)
    ...
    if num_rows > max_rows:
        ...
        y_sample = train_df['label'].values[indices]
        tvec_weights_sample = tvec_weights_train[indices, :]
    else:
        tvec_weights_sample = tvec_weights_train
        y_sample = train_df['label'].values
    
    def eval_k(k, seed):
        nonlocal best_score, best_kmeans, best_homogeneity, smallest_cluster_size
        kmeans = MiniBatchKMeans(n_clusters=k, n_init=10, batch_size=10000, random_state=seed).fit(tvec_weights_sample)
        ...
        print(csv_log_file, split_test_train, 'k', k, 'smallest cluster', smallest_cluster_size,
              'ch', ch_score, 'db', db_score, 'score', score, 'homogeneity', homogeneity)
        
    # automatically determining optimal k. Repeat k-means until small_cluster_size < max_smallest_cluster_size
    while k < 50 or smallest_cluster_size > max_smallest_cluster_size:
        eval_k(k, 0)
        k=int(k*1.1)+1
    #best_k = len(best_kmeans.cluster_centers_)
    #k = best_k - 2
    #while k < best_k + 2:
    #    for seed in range(random_state+1, random_state+5):
    #        eval_k(k, seed)
    #    k+=1
...
def redo_clusters():
    ...
    for file in cluster_files:
        ...
        if not file.endswith(suffix):
            continue
        print(file)
        cluster_df = pd.read_csv(OUTPUT_DIR + file,index_col=[0]).dropna(subset=['tfidf_text'])
        num_clusters = max(cluster_df.clusters) + 1
